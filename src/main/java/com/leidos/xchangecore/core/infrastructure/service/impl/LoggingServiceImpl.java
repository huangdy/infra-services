...
import com.leidos.xchangecore.core.infrastructure.rule.RuleEngine;
...
import com.leidos.xchangecore.core.infrastructure.service.LoggingService;
import com.leidos.xchangecore.core.infrastructure.status.ComponentLogMonitor;
...
public class LoggingServiceImpl
    implements LoggingService, ServiceNamespaces, InitializingBean, StatusEventListener {
    ...
    private final Map<String, ComponentLogMonitor> loggers = new HashMap<String, ComponentLogMonitor>();

    private Map<String, Status> componentStatusMap = new HashMap<String, Status>();
    private final Map<String, StatusEvent> componentStatusEventMap = new HashMap<String, StatusEvent>();
    ...
    private final Map<String, org.apache.log4j.Logger> componentLogOutputs = new HashMap<String, org.apache.log4j.Logger>();

    public RuleEngine ruleEngine;

    private boolean sendHistory = false;

    private final List<String> registeredJidList = new ArrayList<String>();

    private final List<String> localCoreJids = new ArrayList<String>();
    ...
    private final Map<String, Status> coreStatusMap = new HashMap<String, Status>();
    ...
    public GetLogByHostnameResponseDocument getLogByHostname(GetLogByHostnameRequestDocument request) {
        ...
        String hostname;
        int numOfRecords = 0;
        GetLogByHostnameResponseDocument response = GetLogByHostnameResponseDocument.Factory.newInstance();
        ...
        if (request.getGetLogByHostnameRequest().getHostname() != null) {
            ...
            GetLogByHostnameResponse getlogByHostnameResponse = response.addNewGetLogByHostnameResponse();
    ...
    public GetLogByLoggerResponseDocument getLogByLogger(GetLogByLoggerRequestDocument request) {
        ...
        String loggerStr;
        int numOfRecords = 0;
        GetLogByLoggerResponseDocument response = GetLogByLoggerResponseDocument.Factory.newInstance();
    ...
    public void handleStatusEvent(StatusEvent event) {
        ...
        if (monitor != null) {
            ...
            if (componentTimeStamp != null) {
                if (componentTimeStamp.before(thresholdTimeStamp)) {} else {
            ...
            if (event != null) {
                ...
                if (getCommunicationsService() != null) {
                    for (String localCoreJid : jids) {
                        getCommunicationsService().sendXMPPMessage(xmppMessage,
                            "message",
                            "message",
                            localCoreJid);
                    }
    ...
    private void sendCoreStatus(String coreName, String coreStatus) {
        ...
        if (coreStatus.equalsIgnoreCase("online")) {
            String jid = directoryService.getCoreName();
            ...
            if (resource != null) {
                getCommunicationsService().sendXMPPMessage(message,
                    "message",
                    "message",
                    coreName + "/" + resource);
            }
    ...
    private void sendCoreStatusXmppMessage(Status status,
                                           StatusEvent event,
                                           String localCoreJid,
                                           String coreJid) {
        ...
        if (!coreEventMessage.equals("")) {
            ...
            StatusEvent coreStatusEvent = new StatusEvent("CORE",
                                                          event.getEvent(),
                                                          coreEventMessage,
                                                          event.getTimestamp());
    ...
    public void setLocalCoreJid(String localCoreJid) {
        ...
        List<String> jids = localCoreJids;
        localCoreJids.add(localCoreJid);
        this.localCoreJid = localCoreJid;
        directoryService.setLocalCoreJid(localCoreJid);
        directoryService.setConsoleResource(getConsoleResource());
        initializeStatus();
        sendComponentXmppMessage(jids);
        notifyOtherCores();
        if (localCoreJids.size() > 1) {
    ...
    public void systemInitializedHandler(String messgae) {
        ...
        directoryService.registerUICDSService(NS_LoggingService,
